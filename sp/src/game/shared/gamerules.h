...
enum // weapon respawning return codes
{	
	GR_NONE = 0,
...
enum // Player relationship return codes
{
	GR_NOTTEAMMATE = 0,
	GR_TEAMMATE,
	GR_ENEMY,
...
public:
	...
	virtual int UpdateTransmitState( void ); // ALWAYS transmit to all clients.
...
public:
	...
	{
		CGameRulesProxy::NotifyNetworkStateChanged(); //Forward the call to the entity that will send the data.
	}
	...
	virtual bool AllowThirdPersonCamera( void ) { return false; } //Allow thirdperson camera.
...
public:
	virtual void LevelShutdown( void ) { return; }; // Called when game rules are destroyed by CWorld
	virtual void Precache( void ) { return; };
	virtual void RefreshSkillData( bool forceUpdate );// fill skill data struct with proper values
	virtual void FrameUpdatePostEntityThink(); // Called each frame. This just forwards the call to Think().
	...
	virtual void EndGameFrame( void ); // Called at the end of GameFrame (i.e. after all game logic has run this frame)
